...
function MyGroupApply({
  ...
  return (
    <>
      ...
      <td className="md:p-2 min-w-[40px]">{groupApply.user_name}</td>
      ...
      <td className="md:p-2 h-[44px] text-center flex justify-center items-center">
        <div onClick={paidGroupApplyHandler} className="w-6 h-6 cursor-pointer">
          {isPaid ? (
            <Image src="/img/icon-checkbox-checked.png" alt="체크된 체크박스" width={24} height={24} />
          ) : (
            <Image src="/img/icon-checkbox.png" alt="체크 전 체크박스" width={24} height={24} />
          )}
        </div>
      </td>
    </>
