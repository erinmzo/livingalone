...
async function GroupDetail({ params }: Props) {
  ...
  const achievementRate = Math.round((group_applications.length / people_num) * 100);
      ...
      <InnerLayout>
        <div className={`${is_finished ? "text-gray-3" : ""}`}>
          ...
          <div className="flex justify-between mt-[23px]">
            ...
            <div className="flex">
              <p className={`text-[20px] ${is_finished ? "text-gray-2" : "text-red-3"}`}>
                달성률 <span className="font-bold text-[36px]">{achievementRate}%</span>
              </p>
            </div>
          </div>
          <div className="mt-5">
            ...
            <p className={`font-bold text-[24px] mt-3 mb-[4px]  ${is_finished ? "text-gray-3" : "text-black"}`}>
            ...
            {/* 임시 공유 버튼 */}
            <ShareButton postId={data.id} title={title} content={item} imgUrl={img_url} />
