export const phoneValidate = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (rawPhoneNumber.length <= 2) {
    ...
    formattedPhoneNumber = `${rawPhoneNumber.slice(0, 3)}-${rawPhoneNumber.slice(3)}`;
  } else {
    formattedPhoneNumber = `${rawPhoneNumber.slice(0, 3)}-${rawPhoneNumber.slice(3, 7)}-${rawPhoneNumber.slice(7, 11)}`;
  }
